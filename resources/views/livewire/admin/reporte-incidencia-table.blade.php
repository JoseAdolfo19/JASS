<div class="w-full py-8 px-4 sm:px-6 lg:px-8" x-data="incidenceTable()">
    <!-- Notificaciones -->
    @if (session('success'))
        <script>
            Swal.fire({
            ...
            <table class="w-full">
                <thead class="bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-zinc-300 uppercase">Nombre Asociado</th>
                ...
                <tbody class="divide-y divide-zinc-800">
                    @foreach ($incidences as $incidence)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ Str::limit($incidence->associate->name, 20) }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ Str::limit($incidence->location, 20) }}</td>
                            ...
                            <td class="px-4 py-4 text-sm text-right">
                                <!-- Botón Editar -->
                                <button
                                    @click="openModal({{ $incidence->id }}, '{{ addslashes($incidence->associate->id) }}', '{{ addslashes($incidence->description) }}', '{{ addslashes($incidence->type_incidence) }}', '{{ $incidence->date_reported }}')"
                                ...
                                <!-- Botón Eliminar -->
                                <button onclick="confirmDelete({{ $incidence->id }})" class="text-red-500 hover:text-red-400">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <form id="delete-form-{{ $incidence->id }}" action="{{ route('admin.reported_incidence.destroy', $incidence->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
        ...
        <div x-show="isOpen" x-cloak x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-zinc-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-zinc-800">
                    <form :action="'/admin/reported_incidence/' + currentId" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="px-8 py-8">
                            ...
                            <!-- Campo ID Asociados -->
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-zinc-300 mb-2">Nombre Asociado</label>
                                <input type="number" x-model="currentNombreAsociado" name="id_associates"
                            ...
                            <!-- Campo Fecha Reporte -->
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-zinc-300 mb-2">Fecha de Reporte</label>
                            ...
                            <div class="mb-6">
                                ...
                                <select name="status"
                                    class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                    <option value="pendiente" :selected="currentTypeIncidence === 'pendiente'">Pendiente</option>
                                    <option value="resuelto" :selected="currentTypeIncidence === 'resuelto'">Resuelto</option>
                                </select>
                            </div>
                        </div>
